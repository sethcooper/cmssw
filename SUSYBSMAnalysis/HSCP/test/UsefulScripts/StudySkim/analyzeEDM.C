...
void analyzeEDM(std::string fileName, std::string cwdPath="./")
{
   ...
   for(ls.toBegin(); !ls.atEnd(); ++ls){
       ...
       if(!nEventsSkimmedCounter.isValid()){printf("Invalid nEventsSkimmedCounterH\n");continue;}
   ...
   FILE* pFile = fopen((cwdPath + "/Results/" + fileName + ".txt").c_str(), "w");
      fprintf(pFile, "%60s : Skimmed/Total = %6.0f/%6.0f = %6.2f%%  EventSize=%6.2fKB\n", fileName.c_str(), Skimmed,Total, 100.0*Eff, Size);
   fclose(pFile);
}
