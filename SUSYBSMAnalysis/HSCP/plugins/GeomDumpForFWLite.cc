...
void
GeomDumpForFWLite::beginRun(edm::Run& run, const edm::EventSetup& iSetup)
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
GeomDumpForFWLite::endJob()
{
}
...
void
GeomDumpForFWLite::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   iSetup.get<MuonGeometryRecord>().get( RpcGeom );
      ...
      for(unsigned int i=0;i<TkDets.size() ;i++){MuonDets.push_back(TkDets [i]);}
      ...
      for(unsigned int i=0;i<MuonDets.size();i++){
         ...
         if(Detid.det()==1 && (SubDet<1 || SubDet>6)) continue;
	    ...
	    std::array<const float, 4> const & parameters = (*trapezoidalBounds).parameters();
         ...
         posV->SetX(Pos.x()); posV->SetY(Pos.y()); posV->SetZ(Pos.z());
         widthV->SetX(WidthVector.x() -Pos.x()); widthV->SetY(WidthVector.y() -Pos.y()); widthV->SetZ(WidthVector.z() -Pos.z());
         lengthV->SetX(LengthVector.x()-Pos.x()); lengthV->SetY(LengthVector.y()-Pos.y()); lengthV->SetZ(LengthVector.z()-Pos.z());
         thickV->SetX(ThickVector.x() -Pos.x()); thickV->SetY(ThickVector.y() -Pos.y()); thickV->SetZ(ThickVector.z() -Pos.z());
